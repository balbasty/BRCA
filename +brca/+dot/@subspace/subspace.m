classdef subspace < brca.dot.base
% dot_subspace = brca.dot.subspace()
%   Inner product based on a known covariance of the form U*U' + inv(lam*L).
%   Such a covariance may have been learned beforehand by BRCA.

    properties
        Subspace                        % Principal subspace
        Precision = 1                   % Residual precision     [1]
        Dot       = brca.dot.euclidean  % Subspace inner product [euclidean]
    end
    ...
    methods
        ...
        % dot_subspace = brca.dot.subspace(Subspace,[Precision],[Dot])
        ...
        %   Returns L*x, where L is the underlying precision matrix.
            ...
            WLx = zeros(obj.M,1);
            for m=1:obj.M
                Um = brca.format.read(obj.Subspace);
                WLx(m) = Um(:)' * Lx(:);
                clear Um
            end
            WLx = obj.P * WLx;
            for m=1:obj.M
                LUm = obj.dot.solve(brca.format.read(obj.Subspace));
                Lx = Lx - WLx(m) * LUm;
                clear LUm
            end
            Lx = obj.Precision * Lx;
    ...
    % Methods that pre-compute useful values
        ...
        function computeULU(obj)
            obj.ULU = zeros();
            for m=1:obj.M
                Um       = brca.format.read(obj.U, m);
                LUm      = obj.dot.solve(Um);
                obj.ULU(m,m) = double(LUm(:))' * double(Um(:));
                clear Um
                for p=(m+1):obj.M
                    Up       = brca.format.read(obj.U, p);
        ...
        function computeP(obj)
            ...
            obj.P = brca.utils.invPD(obj.P);
        end

        function computeM(obj)
            dim = brca.format.size(obj.Subspace);
            obj.M = dim(end);
        end
