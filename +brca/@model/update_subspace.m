function obj = update_subspace(obj)
    ...
    obj.iAu  = brca.utils.invPD(obj.Au);
    ...
    mu = double(brca.format.read(obj.mu));
    [data, obj.data] = deal(obj.data, []);
    for m=1:obj.M
        Um       = 0;
        parfor(n=1:N, obj.parallel)
            data1 = data(n);
            x     = double(brca.format.read(data1.x));
            data1 = [];
            Um    = Um + (x - mu) * ZiA(n,m);
            x     = [];
        end
        obj.U = brca.format.write(obj.U, Um, m);
        clear UM
    end
    obj.data = data;
...
%     for n=1:N
        ...
        Uz    = 0;
        for m=1:M
            Um  = double(brca.format.read(U, m));
            Uz  = Uz + z(m) * Um;
            Um  = [];
        end
        z     = [];
        % b) Compute L * (x- mu)
        Lx = double(brca.format.read(data1.x)) - mu;
        Lx = dot.solve(Lx);
        data1 = [];
        % c) Accumulate
