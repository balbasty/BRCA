classdef nifti < brca.format.base
...
%                 array = subsasgn(array, S, reshape(value, lat));
        ...
        function [type, is_complex] = class(obj, array)
            ...
            [type, is_complex] = brca.utils.type_nii2num(array.dtype);
        end
        ...
        function array = allocate(obj, lat, prefix, type, is_complex)
            ...
            nii_type = brca.utils.type_num2nii(type, is_complex);
            ...
            dim = size(nii.dat);
            for z=1:dim(end)
                nii.dat = brca.format.write(nii.dat, 0, z, numel(dim));
            end
            array = nii;
        end

        function array = like(obj, other, prefix)
            [type, is_complex] = brca.format.class(other);
